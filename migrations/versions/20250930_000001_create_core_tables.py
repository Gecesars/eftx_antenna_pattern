...
sexenum = postgresql.ENUM("M", "F", "X", name=SEXENUM, create_type=False)
pat_enum = postgresql.ENUM("HRP", "VRP", name=PATTERNTYPE, create_type=False)
        ...
        sa.text(
            ...
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = :enum_name) THEN
                    CREATE TYPE sexenum AS ENUM ('M', 'F', 'X');
                END IF;
                IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = :pattern_name) THEN
                    CREATE TYPE patterntype AS ENUM ('HRP', 'VRP');
                END IF;
            END;
            $$
            """
        ).bindparams(enum_name=SEXENUM, pattern_name=PATTERNTYPE)
    )
...
def downgrade() -> None:
    ...
    op.execute(sa.text("DROP TYPE IF EXISTS patterntype CASCADE"))
    op.execute(sa.text("DROP TYPE IF EXISTS sexenum CASCADE"))
